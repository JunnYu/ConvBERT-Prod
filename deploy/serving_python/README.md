...
本文档讲解基于Paddle Serving的ConvBERT模型服务化部署。
...
并使用下面命令，将静态图模型转换为服务化部署模型，即`convbert_server`文件夹中的模型。

```bash
cd deploy/serving_python
python -m paddle_serving_client.convert --dirname  ../../convbert_infer/ --model_filename inference.pdmodel --params_filename inference.pdiparams --serving_server convbert_server --serving_client convbert_client
```
...
```
{'err_no': 0, 'err_msg': '', 'key': ['label_id', 'prob'], 'value': ['[0]', '[0.995919]'], 'tensors': []}
```
