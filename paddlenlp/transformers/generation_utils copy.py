...
class BeamSearchScorer(object):
    ...
    def process(self,
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            beam_idx = 0
            for beam_token_rank, (
                    next_token, next_score, next_index) in enumerate(
    ...
    def finalize(self,
        ...
        for i, beam_hyp in enumerate(self._beam_hyps):
            ...
            for j in range(self.num_beam_hyps_to_keep):
                ...
                sent_lengths[self.num_beam_hyps_to_keep * i + j] = len(
                    best_hyp)
        ...
        if sent_lengths.min().numpy().item() != sent_lengths.max().numpy(
        ).item():
...
class GenerationMixin(object):
        ...
        if decode_strategy == 'greedy_search':
            ...
            if num_return_sequences > num_beams:
                raise ValueError(
                    ...
                    "`num_beams` is {}".format(num_return_sequences,
                                               num_beams))
            if num_beams <= 1:
                raise ValueError(
    ...
    def beam_search(self, input_ids, beam_scorer, logits_processors,
                    max_length, diversity_rate, pad_token_id, eos_token_id,
                    **model_kwargs):
    ...
    def __call__(self, input_ids, scores, current_tokens, beam_group_idx):
        ...
        for batch_idx in range(batch_size):
            ...
            previous_group_tokens = current_tokens[
                ...
                group_start_idx]
            print("previous_group_tokens", previous_group_tokens,
                  previous_group_tokens.shape)
            token_frequency = paddle.histogram(
