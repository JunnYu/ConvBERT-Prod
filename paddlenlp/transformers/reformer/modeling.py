...
ReformerBackwardOutput = namedtuple(
    ...
    [
        "attn_output", "hidden_states", "grad_attn_output",
        "grad_hidden_states"
    ], )
...
class LSHSelfAttention(nn.Layer, EfficientAttentionMixin):
    ...
    def forward(
        ...
        do_standard_self_attention = (
            sequence_length <= self.chunk_length) or (use_cache and
                                                      cache[1] is not None)
    ...
    def _get_relevant_hid_states_and_buckets(
        ...
        max_bucket = (self.num_buckets if isinstance(self.num_buckets, int)
        ...
        relevant_hidden_states = relevant_hidden_states.reshape(shape=[
    ...
    def _gather_by_expansion(self, vectors, idxs, num_hashes):
        ...
        return (paddle.index_sample(vectors, expanded_idxs).reshape(shape=[
            idxs.shape[0], idxs.shape[1], self.attention_head_size, -1
    ...
    def __init__(
        ...
        if len(set(self.attn_layers)) == 1 and self.attn_layers[0] == "lsh":
            self.self_attention = LSHSelfAttention(
                ...
                hidden_size=hidden_size, )
        elif len(set(self.attn_layers)) == 1 and self.attn_layers[
                0] == "local":
