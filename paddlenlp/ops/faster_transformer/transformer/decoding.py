...
def transfer_param(p, is_bias=False, restore_data=False):
    ...
    if restore_data:
        if in_dygraph_mode():
            ...
            new_p = type(p)(shape=param_shape,
                            dtype="float16",
                            is_bias=is_bias)
...
class InferTransformerDecoding(nn.Layer):
    def __init__(self,
        ...
        for arg, value in locals().items():
            if arg not in [
                    "self", "decoder", "word_embedding",
                    "positional_embedding", "linear"
            ]:
            ...
            if paddle.__version__ <= "2.1.3":
                ...
                enc_output = paddle.expand(
                    ...
                    shape=[
                        ...
                        batch_size * self._beam_size, max_seq_len,
                        self._d_model
                    ])
        ...
        if self._use_fp16_decoding:
            ...
            for mod in self._model.encoder.layers:
                ...
                self.sub_modules["slf_q_bias"].append(
                    paddle.concat(
                        [
                            mod.self_attn.q_proj.bias, mod.self_attn.k_proj.
                            bias, mod.self_attn.v_proj.bias
                        ],
                        axis=-1))
                self.sub_modules["slf_k_weight"].append(
                    mod.self_attn.k_proj.weight)
